...
public class RecsCount implements ExecutorInterface{
	...
	public MessageResponse execute(LunarDBServerStandAlone l_db_ssa , MessageRequest msg, Logger logger)
	{
		 ...
		 return recsCount(l_db_ssa, msg, logger);
	}
	
	private MessageResponse recsCount(LunarDBServerStandAlone l_db_ssa , MessageRequest request, Logger logger)
    {
    	...
    	if(params.length < 2)
		{
			...
			response = ExecutorInterface.responseError(request, MessageResponse.getNullStr(), MessageResponse.getNullStr(), CodeSucceed.wrong_parameters_for_records_count);
        	return response; 
		}
		String db = params[0];
